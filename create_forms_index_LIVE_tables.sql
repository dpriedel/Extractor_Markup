-- this will create and index table for the form files
-- downloaded from the SEC EDGAR FTP server.

-- see link below for hints
-- https://wiki.postgresql.org/wiki/Don%27t_Do_This

CREATE SCHEMA IF NOT EXISTS live_forms_index AUTHORIZATION extractor_pg;

DROP TABLE IF EXISTS live_forms_index.sec_form_index CASCADE;

CREATE TABLE live_forms_index.sec_form_index
(
    filing_id BIGINT GENERATED ALWAYS AS IDENTITY UNIQUE,
    cik TEXT NOT NULL,
    accession_number TEXT DEFAULT NULL,
    alternate_id2 TEXT DEFAULT NULL,
    company_name TEXT NOT NULL,
    file_name TEXT DEFAULT NULL,
    symbol TEXT DEFAULT NULL,
    sic TEXT NOT NULL,
    form_type TEXT NOT NULL,
    date_filed DATE DEFAULT NULL,
    period_ending DATE NOT NULL,
    period_context_id TEXT DEFAULT NULL,
    has_html BOOL NOT NULL,
    has_xbrl BOOL NOT NULL,
    has_xls BOOL NOT NULL,
    UNIQUE (cik, form_type, period_ending),
    PRIMARY KEY (cik, form_type, period_ending)
);

ALTER TABLE live_forms_index.sec_form_index OWNER TO extractor_pg;

DROP INDEX IF EXISTS idx_accession_nbr;
CREATE INDEX idx_accession_nbr ON live_forms_index.sec_form_index (accession_number);

DROP INDEX IF EXISTS idx_period_ending;
CREATE INDEX idx_period_ending ON live_forms_index.sec_form_index (period_ending);
