...
protected:
  ...
  std::optional<FileMode> ApplyFilters(const EM::SEC_Header_fields &SEC_fields,
                                       const EM::FileName &file_name,
                                       const EM::DocumentSectionList &sections,
                                       std::atomic<int> *forms_processed);

  bool LoadFileFromFolderToDB(const EM::FileName &file_name,
                              const EM::SEC_Header_fields &SEC_fields,
                              const EM::DocumentSectionList &sections,
                              EM::sv sec_header, FileMode file_mode,
                              std::mutex *db_mutex = nullptr);
  bool LoadFileFromFolderToDB_XBRL(const EM::FileName &file_name,
                                   const EM::SEC_Header_fields &SEC_fields,
                                   const EM::DocumentSectionList &sections,
                                   std::mutex *db_mutex = nullptr);
  bool LoadFileFromFolderToDB_XLS(const EM::FileName &file_name,
                                  const EM::SEC_Header_fields &SEC_fields,
                                  const EM::DocumentSectionList &sections,
                                  EM::sv sec_header,
                                  std::mutex *db_mutex = nullptr);
  bool LoadFileFromFolderToDB_HTML(const EM::FileName &file_name,
                                   const EM::SEC_Header_fields &SEC_fields,
                                   const EM::DocumentSectionList &sections,
                                   EM::sv sec_header,
                                   std::mutex *db_mutex = nullptr);
  bool ExportHtmlFromSingleFile(const EM::DocumentSectionList &sections,
                                const EM::FileName &file_name,
                                EM::sv sec_header);
  void Do_SingleFile(std::atomic<int> *forms_processed, int &success_counter,
                     int &skipped_counter, int &error_counter,
                     const EM::FileName &file_name);

  std::tuple<int, int, int>
  LoadSingleFileToDB(const EM::FileName &input_file_name);
  std::tuple<int, int, int>
  LoadSingleFileToDB_XBRL(const EM::FileContent &file_content,
                          const EM::DocumentSectionList &document_sections,
                          const EM::SEC_Header_fields &SEC_fields,
                          const EM::FileName &input_file_name);
  std::tuple<int, int, int>
  LoadSingleFileToDB_XLS(const EM::FileContent &file_content,
                         const EM::DocumentSectionList &document_sections,
                         EM::sv sec_header,
                         const EM::SEC_Header_fields &SEC_fields,
                         const EM::FileName &input_file_name);
  std::tuple<int, int, int>
  LoadSingleFileToDB_HTML(const EM::FileContent &file_content,
                          const EM::DocumentSectionList &document_sections,
                          EM::sv sec_header,
                          const EM::SEC_Header_fields &SEC_fields,
                          const EM::FileName &input_file_name);
  ...
  std::tuple<int, int, int> LoadFileAsync(const EM::FileName &file_name,
                                          std::atomic<int> *forms_processed,
                                          std::mutex *db_mutex);
...
private:
  ...
  using FilterTypes =
      std::variant<FileHasCIK, FileHasSIC, FileHasXBRL, FileHasFormType,
                   FileHasHTML, FileIsWithinDateRange, NeedToUpdateDBContent>;
