...
void SEC_Header::UseData(EM::FileContent file_content) {
  const boost::regex regex_SEC_header{
      R"***(^<SEC-HEADER>.+?</SEC-HEADER>$)***"};
  boost::cmatch results;

  bool found_it =
      boost::regex_search(file_content.get().cbegin(),
                          file_content.get().cend(), results, regex_SEC_header);
...
void SEC_Header::ExtractCIK() {
  ...
  boost::cmatch results;
  bool found_it = boost::regex_search(header_data_.cbegin(),
                                      header_data_.cend(), results, ex);
...
void SEC_Header::ExtractSIC() {
  ...
  const boost::regex ex{
      R"***(^\s+STANDARD INDUSTRIAL CLASSIFICATION:.+?\[?([0-9]+)\]?)***"};
...
void SEC_Header::ExtractFormType() {
  ...
  parsed_header_data_["form_type"] =
      results.str(1) | rng::actions::transform([](unsigned char c) {
