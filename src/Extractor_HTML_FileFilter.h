...
struct FinancialDocumentFilter {
  ...
  explicit FinancialDocumentFilter(const std::vector<std::string> &forms)
      : forms_{forms} {}
...
struct FinancialStatements {
  ...
  const char *financial_statements_begin_ =
      nullptr;                          // pointer to anchor/first part
...
inline bool operator==(const MultiplierData &lhs, const MultiplierData &rhs) {
  return lhs.multiplier_ == rhs.multiplier_ &&
         lhs.html_document_.get() == rhs.html_document_.get();
}
...
StatementType FindStatementContent(EM::HTMLContent financial_content,
                                   const AnchorsFromHTML &anchors,
                                   const boost::regex &stmt_anchor_regex,
                                   StmtTypeFilter stmt_type_filter) {
  ...
  auto tbl_lookup = FindStatementTableFromAnchor(financial_content, the_anchor,
                                                 stmt_type_filter);
  if (tbl_lookup) {
    ...
    size_t total_len = stmt_tbl.TableContent().get().data() +
                       stmt_tbl.TableContent().get().size() - anchor_begin;
